...
fn handle_message(line: &str, state: &AppState) -> String {
    let msg: JsonRpcMessage = match serde_json::from_str(line) {
        Ok(m) => m,
        Err(e) => return json_rpc_error(serde_json::Value::Null, -32700, &format!("Parse error: {}", e)),
    };
...
fn handle_tools_call(msg: &JsonRpcMessage, state: &AppState) -> String {
    ...
    match result {
        Ok(content) => {
            json_rpc_result(msg.id.clone(), json!({
...
fn get_skill_tool(args: &Value, state: &AppState) -> Result<String, String> {
    ...
    if !skill_json.exists() {
        return Err(format!("Skill '{}' not found. Available skills are indexed under the registry.", path));
    }

    let meta: Value = serde_json::from_str(&fs::read_to_string(&skill_json).map_err(|e| e.to_string())?)
        .map_err(|e| e.to_string())?;

    let mut output = String::new();
    output.push_str(&format!("# {} — {}\n\n", meta["name"].as_str().unwrap_or("?"), meta["repo"].as_str().unwrap_or("?")));
    output.push_str(&format!("Language: {}\n", meta["language"].as_str().unwrap_or("?")));
    output.push_str(&format!("Tier: {}  Group: {}  Trust: {}  Risk: {}\n",
...
fn search_skills_tool(args: &Value, state: &AppState) -> Result<String, String> {
    let query = args.get("query").and_then(|q| q.as_str()).ok_or("Missing 'query' argument")?.to_lowercase();
...
fn find_skills_tool(args: &Value, state: &AppState) -> Result<String, String> {
    ...
    let mut output = format!("{} results for '{}':\n\n", results.len(), query);
    for (i, (key, score)) in results.iter().enumerate() {
        let pct = if max_score > 0.0 { (score / max_score * 100.0) as i32 } else { 0 };
        output.push_str(&format!("{}. {} (relevance: {}%)\n", i + 1, key, pct));
    }
...
fn get_section_tool(args: &Value, state: &AppState) -> Result<String, String> {
    let path = args.get("path").and_then(|p| p.as_str()).ok_or("Missing 'path' argument")?;
    let section = args.get("section").and_then(|s| s.as_str()).ok_or("Missing 'section' argument")?;

    let section = if section.ends_with(".md") { section.to_string() } else { format!("{}.md", section) };
    let file_path = state.registry.join("skills").join(path).join(&section);

    if !file_path.exists() {
        return Err(format!("Section '{}' not found in skill '{}'", section, path));
    }
...
fn collect_and_score(dir: &std::path::Path, prefix: String, query_tokens: &[String], results: &mut Vec<(String, f64)>) -> Result<(), String> {
            ...
            for entry in entries.flatten() {
                ...
                if path.is_dir() && path.file_name().map_or(false, |n| n == "examples") {
                    ...
                    if let Ok(examples) = fs::read_dir(&path) {
                        let count = examples.flatten().filter(|e| e.file_type().map_or(false, |t| t.is_file())).count();
...
fn find_registry() -> PathBuf {
    ...
    if cache_dir.exists() { return PathBuf::from(&home).join(".libskills"); }
